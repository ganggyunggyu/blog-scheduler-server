...
- 블로그 발행 계정 source of truth 는 scheduler-server 기본 MongoDB 의 `blogaccounts` 컬렉션임.
- `cafe-bot.accounts` 의 `isActive` 는 카페 운영 기준이므로 블로그 발행 대상 판단에 쓰지 않음.
...
- mongosh 호출 예: `mongosh --quiet "$(grep ^MONGO_URI= .env | cut -d= -f2-)" --eval 'db.blogaccounts.find().sort({ category: 1, nickname: 1 })'`
- `blogaccounts` 에는 accountId/blogId/nickname/category/isEnabled/status/note 같은 메타데이터만 저장하고, 비밀번호는 저장하지 않음. 실행 비밀번호는 스케쥴 payload/BullMQ job payload 에만 둠.
...
- 스케쥴 생성/예약 후에는 해당 도메인의 `.claude/commands/schedule-*.md` 포맷 체크리스트 기준으로 각 글을 직접 열어 검증함. 제목/본문 구조, 카테고리, 예약 시각, 링크/CTA, 이미지 개수/순서/위치, 대표이미지, 캡션/alt/metadata 등 도메인별 필수 요소가 실제 네이버 UI에 맞게 들어갔는지 확인함.
- 이미지 metadata 와 삽입 규칙은 도메인/계정마다 다르므로 다른 스케쥴 포맷으로 추론하지 않음. 해당 command 문서의 포맷과 실제 예약글 화면이 일치할 때만 검증 완료로 봄.

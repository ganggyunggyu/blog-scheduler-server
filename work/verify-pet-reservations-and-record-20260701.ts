...
const OUTPUT_DIR = path.resolve(process.cwd(), 'outputs', `reservation-verify-${TARGET_DATE}-pet`);
...
const onlyServices = new Set(
  getArg('--services', 'pet-sheet-daily')
    .split(',')
    .map((item) => item.trim())
    .filter(Boolean),
);
...
const buildTimeCandidates = (time: string): string[] =>
  time ? [time, addMinutesToTime(time, -1)] : [];
...
const getCookiesForAccount = async (
  ...
  if (!password) throw new Error('no cached session or runtime password');
...
const verifyAccount = async (
  ...
  const expectedTimeCandidates = expectedTimes.map(buildTimeCandidates);
  ...
  try {
    ...
    return {
      ...
      timeHits: expectedTimes.filter((time, index) =>
        time && expectedTimeCandidates[index].some((candidate) => popupText.includes(candidate))),
      debugTextPath,
      screenshotPath,
    };
  } catch (error) {
...
const main = async (): Promise<void> => {
  ...
  const rows = targetJobs.flatMap((item) => {
    ...
    return [[
      ...
      `${item.schedule.service ?? 'unknown'} / 7월 1일 계정당 2건 예약`,

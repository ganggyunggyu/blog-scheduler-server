...
- 애견 입력과 출력 키워드 텍스트 파일은 `Daeyoon_NAS` 에서 관리함
- 현재 macOS 마운트 기준 입력 경로는 `/Volumes/21lab_데이터관리/0_자동발행/0_애견자동발행/애견_0507` 임
- 현재 macOS 마운트 기준 출력 경로는 `/Volumes/21lab_데이터관리/0_자동발행/0_애견자동발행/애견_0507_출력` 임
- 애견 스케쥴 입력을 파일/폴더에서 읽을 때는 입력 경로를 쓰고, 키워드 텍스트 결과물은 출력 경로에 저장함
- 이미지 프로세서를 돌릴 때 출력 경로에 해당 결과물이 이미 있으면 같은 작업을 다시 실행하지 않고 기존 출력물을 재사용함
- 이미지 프로세서 출력 검토에서 매핑, 이미지 수, metadata, 키워드 txt 에 이상이 없으면 사용자 재확인 없이 S3 업로드까지 진행함
- 사용자가 `21일`, `0521`, `5월 21일`처럼 날짜를 지정하면 위 0507 기본 경로를 그대로 쓰지 말고, 같은 NAS 루트의 `애견_MMDD_출력/키워드_카테고리.txt` 를 기준으로 스케쥴 payload 를 조립함
- `temp-image-gen` 에서 만든 `키워드_카테고리.txt` 를 source of truth 로 보고, 블로그명 단락과 `키워드:카테고리` 값을 그대로 사용함
- 발행 전에는 각 `product-images/{blogId}/{keyword}/라이브러리제외` 또는 `라이브러리제외이미지` S3 객체가 1개 이상 있는지 확인함
- `GET /api/image/product-images` 는 정상 조회 후 S3 폴더를 `_used_...` 로 이동시키므로, 발행 전 검증만 하려고 직접 호출하지 않음. 실제 API 호출은 `/bot/auto-schedule` 로 등록된 generate worker 가 수행하게 둠
- 라이브러리제외 이미지가 0장인 계정/키워드가 있으면 스케쥴 등록을 시작하지 않고 이미지 소스/S3 업로드를 먼저 보강함
- 스케쥴 서버 payload 의 `keyword_category` 는 에디터 분기값으로 `애견` 을 유지하고, 원고/이미지 생성용 세부 분류는 `keywords` 의 `키워드:카테고리` 에 포함함

...
- **같은 메인 키워드가 전체 분배의 50% 이상을 차지하면 안 됨**
- 메인 키워드 = 키워드 앞쪽 공통 어절 (예: `임산부*`, `소음인*`, `수족냉증*`, `임신초기*`)
...
2. 필요한 N개를 **그룹별 라운드로빈**으로 추출함
   - 예: N=30, 그룹 6개라면 그룹마다 5개씩
3. 한 메인 키워드 그룹이 **전체의 50% 초과 시 자동 거부** → 더 다양화해서 다시 추출
4. 각 계정에는 **서로 다른 그룹의 키워드가 섞이도록** 분배함 (한 계정이 임산부만 10개 X)

...
### ⚠️ 메인 키워드 다양성 규칙 (필수, 더 엄격)

- **수족냉증* / 수족냉증증상 / 수족냉증어쩌고 류 반복 절대 금지** — 이미 사이클1에서 30개 이상 소진했으므로 사이클2/3에서는 변형 포함 어떤 형태로도 재사용 금지
- **같은 메인 키워드 그룹은 한 회차(같은 날 묶음) 전체에서 1회만 허용** — 50% 룰이 아니라 그룹당 1개 상한
- **한 계정에 같은 메인 키워드 루트 연속 배정 금지** — 어제/오늘 발행한 메인이 무엇이었는지 RSS로 먼저 확인하고 다른 그룹으로 배정
- 메인 키워드 = 키워드 앞쪽 공통 어절 (예: `임산부*`, `소음인*`, `수족냉증*`, `임신초기*`, `손발*`, `갱년기*`)
- 시트는 카테고리(임산부/소음인/임신초기/흑염소진액/공진단/홍삼 등)로 그룹화되어 있어, 앞에서부터 N개를 그대로 뽑으면 한 그룹이 몰림 → **반드시 그룹별로 골고루 섞어서 뽑아야 함**

추출 절차:
1. 미사용 키워드 전체 리스트에서 **메인 키워드 그룹**으로 분류함
   - 예: `임산부*` 그룹, `소음인*` 그룹, `흑염소진액*` 그룹, `임신*` 그룹, `갱년기*` 그룹 등
2. **금지 그룹 제외**: `수족냉증*`, `손발*저림*`, `손발*차가*` 류는 이미 소진 → 신규 사이클에서 후순위 (최후의 1~2개만 허용)
3. 필요한 N개를 **그룹별 라운드로빈**으로 추출함, **한 그룹당 1개만** 뽑음
4. 한 메인 키워드 그룹이 **전체의 2개 이상이면 자동 거부** → 더 다양화해서 다시 추출
5. 각 계정에는 **서로 다른 그룹의 키워드가 섞이도록** 분배함 (한 계정이 임산부만 10개 X)
6. 보충 스케쥴(1~2개)일 때도 같은 룰 적용 — 그 날 다른 흑염소 계정이 발행한 메인과 안 겹치게

**Why:** 같은 메인 키워드를 한 계정이 5개 넘게 받으면 SEO/주제 다양성 측면에서 차이를 만들기 어렵고, 같은 시기 같은 메인 키워드가 사이트별로 몰리면 외부에서 보기에도 부자연스러움.

**How to apply:** 키워드 추출 단계에서 **그룹별로 뽑는 로직을 반드시 거칠 것**. 시트 앞쪽부터 N개를 그냥 자르는 방식 금지.
